...
With one of Dionysius' steps of understanding a text accounted for, we only have five to go. However, while the first is definitely the most fundemental, and the sixth relies upon the other five as a basis, the remaining four objects in the list comprise a singular step towards understanding, and can be tackled in any order. This step, as it appears in Table A, is the Scholarly Argument step. [Example 1, Example 2, Example 3, Example 4].
